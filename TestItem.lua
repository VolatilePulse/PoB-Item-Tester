...
print("Build:  "..(arg[1] or "Builds\\Zivhi.xml"))
print("Input:  "..(arg[2] or "testitem.txt"))
print("Output: "..((arg[2] or "testitem")..".html"))
...
function lineToHtml(txt)
	return txt:gsub("^%^7", ""):gsub("%^x(......)", "<span style=\"color:#%1\">"):gsub("%^7", "</span>")
end
...
-- Load a build file
local buildXml = loadText(arg[1] or "Builds\\Zivhi.xml")
loadBuildFromXML(buildXml)

-- Load an item from copy data
local itemText = loadText(arg[2] or "testitem.txt")
...
if newItem.base then
	...
	local outFile = io.open((arg[2] or "testitem")..".html", "w")
	if outFile then
...
-- Pause so console output can be read
-- io.read()
